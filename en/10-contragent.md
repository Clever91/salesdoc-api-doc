...
To find out which server you are working with, rely on the method behavior described below: if the `setContragent` / `getContragent` methods are available and a `contragent` object appears in payment responses, this is a **contragent server**.
...
- **Clients** are the **salepoints** (`clients`) of the contragent, where sales actually happen.
...
- **Payments and opening balances** sent for a client (salepoint) are automatically attributed to that client's contragent. Responses then return **both** objects: `contragent` identifies the **contragent** (payer), while `client` identifies the outlet the operation belonged to.
...
> - payment attribution and the `contragent` object — [`setPayment`](09-finance-photo-extra.md#131-setpayment--create-payment), [`getPayment`](02-get-references.md#915-getpayment--payments);
...
| **clients** | array | Contragent salepoints |
| `clients[].CS_id` | string | Salepoint external identifier |
| `clients[].SD_id` | string | Salepoint server ID |
| `clients[].code_1C` | string | Salepoint 1C code |
| `clients[].name` | string | Salepoint name |

> For a contragent without salepoints the `clients` array is returned empty (`[]`) — such a contragent is still present in the list.
