...
| `brand` | object | ❌ | Бренд |
| `description` | string\|null | ❌ | Основное описание товара, до 1000 символов |
| `description_ru` | string\|null | ❌ | Описание на русском языке, до 1000 символов |
| `description_uz` | string\|null | ❌ | Описание на узбекском языке, до 1000 символов |

#### Описания товара: правила обновления

Поля `description`, `description_ru` и `description_uz` необязательны и независимы: изменение одного поля не влияет на остальные. Правила одинаковы для всех трёх полей.

| Что передано в элементе | Результат |
|-------------------------|-----------|
| Ключ отсутствует | Сохранённое значение не меняется |
| Строка | Значение сохраняется (пробелы по краям для `description_ru` и `description_uz` обрезаются) |
| `null` или `""` | Значение очищается. Для `description_ru` и `description_uz` метод `getProduct` вернёт `null` |

- Сервер **не копирует** `description` в `description_ru` или `description_uz`. Если перевод не передан, он остаётся пустым.
- Интеграции, которые не передают эти поля, работают как прежде: существующие описания не удаляются.
- Товар и его описания сохраняются в одной транзакции: если описания не удалось сохранить, изменения товара тоже отменяются.
- Метод `setProduct` в `/api/v4` принимает те же поля с теми же правилами.

**Частичное обновление** — изменить только узбекское описание, остальные поля не трогать:
```json
{
    "method": "setProduct",
    "auth": { "userId": "d0_1", "token": "..." },
...
**Ошибки валидации.** Если `description_ru` или `description_uz` длиннее 1000 символов или не является строкой, элемент не сохраняется и учитывается в `error`. Остальные элементы пакета обрабатываются как обычно. Пример элемента с ошибкой:
```json
{
    "CS_id": null,
    "SD_id": "d0_15",
