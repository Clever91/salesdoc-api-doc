# 📘 GET-методы — Дополнительные (9.32–9.51)
...
**Описание:** Получение фотоотчётов агентов. **По умолчанию** (без фильтра по
дате) возвращаются отчёты **только за сегодня**. Чтобы получить отчёты за
определённый период, передайте `params.filter.period.date` с **обоими** полями
`from` и `to` — результат охватит включительный диапазон дат `[from, to]`. Для
одного дня укажите `from == to`.

**Параметры фильтра по дате:**

| Параметр | Тип | Формат | Описание |
|------|-----|--------|----------|
| `filter.period.date.from` | string | `YYYY-MM-DD` | Начало диапазона (включительно). Обязателен вместе с `to` для режима диапазона. |
| `filter.period.date.to` | string | `YYYY-MM-DD` | Конец диапазона (включительно). Обязателен вместе с `from`. |

> Если `from` или `to` не является корректной датой формата `YYYY-MM-DD`, запрос
> отклоняется (см. **Ошибку валидации** ниже). Если фильтр `date` не передан,
> возвращаются только сегодняшние отчёты (обратная совместимость).

**Запрос (по умолчанию — только сегодня):**
```json
{
...
**Ошибка валидации** (некорректные `from`/`to`):
```json
{
    "status": false,
    "result": null,
...
**Описание:** Получение списка тегов из справочника `Tag`.
...
### 9.48. `getMovementFilialStatus` — Статусы межфилиальных перемещений

**Описание:** Получение списка межфилиальных перемещений с их текущим статусом. В отличие от `getMovementBetweenFilial` (который возвращает состав перемещения), этот метод возвращает статус движения между филиалом-отправителем и филиалом-получателем.

> ⚠️ Для этого метода **обязателен** `filial_id` в запросе. Если он не указан — возвращается ошибка `400` с сообщением `Для этого метода требуется filial_id`.

**Фильтры:**

| Фильтр | Описание |
|--------|----------|
| `filter.period.date.from/to` | По дате перемещения (формат `Y-m-d`) |

**Ограничения:** поддерживаются `limit` и `page` (постраничный вывод).
...
| Поле | Тип | Описание |
|------|-----|----------|
| `CS_id` | string | Внешний идентификатор перемещения (с префиксом) |
| `SD_id` | string | Серверный ID перемещения |
| `code_1C` | string | Код перемещения в 1С |
| **from_filial** | object | Филиал-отправитель |
| `from_filial.id` | integer | ID филиала |
| `from_filial.name` | string | Название филиала |
| `from_filial.filial_id` | string | Внешний код филиала (XML_ID) |
| **to_filial** | object | Филиал-получатель |
| `to_filial.id` | integer | ID филиала |
| `to_filial.name` | string | Название филиала |
| `to_filial.filial_id` | string | Внешний код филиала (XML_ID) |
| `status` | string | Статус перемещения (см. таблицу ниже) |
| `comment` | string | Комментарий |

**Статусы перемещения (`status`):**

| Значение | Описание |
|----------|----------|
| `draft` | Черновик |
| `pending` | Отправлено, ожидает подтверждения |
| `approved` | Подтверждено (создана закупка на стороне получателя) |
| `rejected` | Отклонено |
| `cancel` | Отменено |
...
| Ошибка | Код | Причина |
|--------|-----|---------|
| `Для этого метода требуется filial_id` | 400 | В запросе не указан `filial.filial_id` |
| `Дата начала должна быть меньше даты конца` | 400 | `from` больше `to` |
| `` Дата должна быть в формате `Y-m-d` `` | 400 | Неверный формат даты в фильтре периода |

---

### 9.49. `getAgentLastTime` — Последние координаты агентов

**Описание:** Получение времени последней GPS-записи по каждому агенту. Для каждого агента возвращается время последней записи в журнале GPS (`LAST_TIME`) и одна из координат агента из журнала (учитываются только записи с положительными `LAT` и `LON`; координата может не совпадать с самой последней записью).
...
| `LAT` | string | Широта координаты агента |
| `LON` | string | Долгота координаты агента |
| `LAST_TIME` | string | Дата и время последней GPS-записи агента |

> **Примечание:** Метод возвращает данные «как есть» из базы, поэтому числовые поля (`LAT`, `LON`) могут приходить строками — приводите их к нужному типу на стороне интегратора. Имена полей возвращаются в верхнем регистре без изменений.

---

### 9.50. `getSubstatus` — Справочник дополнительных статусов

**Описание:** Получение справочника дополнительных статусов заказов. Дополнительный статус — это уточнение основного статуса заказа: каждый дополнительный статус относится ровно к одному основному статусу (1–5). У заказа может быть не более одного дополнительного статуса; изменение дополнительного статуса никогда не меняет основной статус заказа. Пагинации нет — справочник возвращается целиком.
...
| Поле | Тип | Описание |
|------|-----|----------|
| `id` | integer | Идентификатор дополнительного статуса |
| `name` | string | Название дополнительного статуса |
| `parent_status` | integer | Основной статус заказа, к которому относится дополнительный статус (1 — новый, 2 — отправлен, 3 — доставлен, 4 — закрыт, 5 — отменён) |

> **Как определить, какие значения доступны конкретному заказу:**
> 1. Возьмите основной статус заказа (`status`) и его текущее поле `additionalStatus` из [`getOrder`](04-get-orders.md#926-getorder--заказы).
> 2. Установить можно любой дополнительный статус, у которого `parent_status` совпадает с основным статусом заказа, — независимо от того, какой дополнительный статус установлен у заказа сейчас (и установлен ли вообще).
> 3. Значение `0` («снять дополнительный статус», оставить заказ без него) доступно всегда.
> 4. Повторная установка уже установленного значения — идемпотентная операция: [`setSubstatus`](08-set-warehouse-orders.md#127-setsubstatus--изменить-дополнительный-статус-заказа) вернёт успех, не изменяя заказ.

> **Примечание:** установить или снять дополнительный статус заказа можно методом [`setSubstatus`](08-set-warehouse-orders.md#127-setsubstatus--изменить-дополнительный-статус-заказа); история изменений доступна через [`getSubstatusLog`](#951-getsubstatuslog--история-изменений-дополнительных-статусов); в ответе [`getOrder`](04-get-orders.md#926-getorder--заказы) дополнительный статус возвращается в поле `additionalStatus`.

---

### 9.51. `getSubstatusLog` — История изменений дополнительных статусов

**Описание:** Получение истории изменений дополнительных статусов заказов. Поддерживается стандартная пагинация. Оба фильтра необязательны и могут комбинироваться; без фильтров возвращается вся история.

**Фильтры:**

| Фильтр | Описание |
|--------|----------|
| `filter.order` | По конкретному заказу (`CS_id` / `SD_id` / `code_1C`) |
| `filter.period.dateUpdate.from/to` | По дате изменения |
...
| Поле | Тип | Описание |
|------|-----|----------|
| **order** | object | Заказ, у которого изменился дополнительный статус |
| `order.CS_id` | string | Внешний идентификатор заказа |
| `order.SD_id` | string | Внутренний идентификатор заказа в SalesDoc |
| `order.code_1C` | string | Код заказа в 1С |
| `oldAdditionalStatus` | object\|null | Дополнительный статус до изменения (`{id, name}`); `null`, если дополнительный статус не был установлен |
| `newAdditionalStatus` | object\|null | Дополнительный статус после изменения (`{id, name}`); `null`, если дополнительный статус был снят |
| `date` | string | Дата и время изменения (`YYYY-MM-DD HH:MM:SS`) |
| `user` | object\|null | Пользователь, выполнивший изменение (`SD_id`, `login`, `name`); `null`, если пользователь неизвестен |

> **Примечание:** если основной статус заказа меняется, а текущий дополнительный статус не относится к новому основному статусу, дополнительный статус снимается автоматически. Такое изменение тоже попадает в историю — как строка с `newAdditionalStatus: null`.
