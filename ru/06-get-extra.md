...
| `status` | integer | Основной статус заказа, к которому относится дополнительный статус (1 — новый, 2 — отправлен, 3 — доставлен, 4 — закрыт, 5 — отменён) |
| `transitions` | array | Массив ID дополнительных статусов, в которые разрешён переход из данного. Значение `0` означает «снять дополнительный статус» (оставить заказ без дополнительного статуса) |
...
> 2. Если `additionalStatus` равен `null` (дополнительный статус ещё не установлен) — установить можно любой дополнительный статус, у которого `status` совпадает с основным статусом заказа.
> 3. Если дополнительный статус уже установлен — доступны только значения из его списка `transitions`. Значение `0` («снять») доступно всегда.
> 4. Повторная установка уже установленного значения не считается переходом: [`setSubstatus`](08-set-warehouse-orders.md#127-setsubstatus--изменить-дополнительный-статус-заказа) вернёт успех, не изменяя заказ.
...
| `source` | string | Источник изменения: `web` — интерфейс SalesDoc, `api` — данный API, `auto` — автоматическое снятие при смене основного статуса заказа |
| `user` | object\|null | Пользователь, выполнивший изменение (`SD_id`, `login`, `name`); `null`, если пользователь неизвестен |

> **Примечание:** если основной статус заказа меняется, а текущий дополнительный статус не относится к новому основному статусу, дополнительный статус снимается автоматически. Такое изменение попадает в историю с `source: "auto"` и `newAdditionalStatus: null`.
