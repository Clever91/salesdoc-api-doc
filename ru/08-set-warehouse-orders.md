...
**Описание:** Установка, изменение или снятие дополнительного статуса одного или нескольких заказов. Дополнительный статус — это уточнение основного статуса заказа; справочник дополнительных статусов и разрешённые переходы возвращает метод [`getSubstatus`](06-get-extra.md#950-getsubstatus--справочник-дополнительных-статусов). У заказа может быть не более одного дополнительного статуса.

> ⚠️ **Основной статус заказа этим методом никогда не изменяется.** Новый дополнительный статус должен относиться к текущему основному статусу заказа, а переход из текущего дополнительного статуса должен быть разрешён его списком `transitions`. Если у заказа ещё нет дополнительного статуса, установить можно любой дополнительный статус текущего основного статуса заказа.
...
| `Переход между доп. статусами запрещён` | Переход из текущего дополнительного статуса в указанный не разрешён списком `transitions` |

> **Примечание:** если основной статус заказа меняется (например, методом `setStatus`) и текущий дополнительный статус не относится к новому основному статусу, дополнительный статус снимается автоматически; такое изменение фиксируется в [`getSubstatusLog`](06-get-extra.md#951-getsubstatuslog--история-изменений-дополнительных-статусов) с источником `auto`.
