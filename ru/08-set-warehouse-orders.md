...
> ⚠️ **Основной статус заказа этим методом никогда не изменяется.** Новый дополнительный статус должен относиться к текущему основному статусу заказа, а переход из текущего дополнительного статуса должен быть разрешён его списком `transitions`.
