...
Дополнительно в каждую оплату добавляется объект `salepoint`, идентифицирующий торговую точку, по которой была проведена оплата (`CS_id` / `SD_id` / `code_1C`). Если оплата не привязана к конкретной торговой точке (например, отправлена напрямую по контрагенту), поля объекта `salepoint` возвращаются как `null`.
...
| Поле | Тип | Описание |
|------|-----|----------|
| **salepoint** | object | Торговая точка, по которой проведена оплата (только на сервере контрагентов) |
| `salepoint.CS_id` | string\|null | Идентификатор торговой точки |
| `salepoint.SD_id` | string\|null | Серверный ID торговой точки |
| `salepoint.code_1C` | string\|null | Код торговой точки в 1С |

На обычном сервере объект `salepoint` в ответе отсутствует, объект `client` идентифицирует клиента (торговую точку), и поведение метода не меняется.
