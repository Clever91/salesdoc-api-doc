...
На **сервере контрагентов** каждая оплата возвращает **оба** объекта: `contragent` идентифицирует **контрагента** (плательщика, в том числе контрагентов без торговых точек), а `client` — торговую точку, по которой была проведена оплата. Если оплата не привязана к конкретной торговой точке (например, отправлена напрямую по контрагенту), поля объекта `client` возвращаются как `null`.
...
| Поле | Тип | Описание |
|------|-----|----------|
| **contragent** | object | Контрагент (плательщик), на котором учтена оплата (только на сервере контрагентов) |
| `contragent.CS_id` | string\|null | Идентификатор контрагента |
| `contragent.SD_id` | string\|null | Серверный ID контрагента |
| `contragent.code_1C` | string\|null | Код контрагента в 1С |
| **client** | object | Торговая точка, по которой проведена оплата; поля равны `null`, если оплата отправлена напрямую по контрагенту |

На обычном сервере объект `contragent` в ответе отсутствует, объект `client` идентифицирует клиента (торговую точку), и поведение метода не меняется.
