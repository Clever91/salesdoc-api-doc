...
SalesDoc API V2 входит в семейство интеграционных API **SalesDoc**. Он использует тот
же конверт запроса/ответа, ту же модель токенов и то же соглашение об идентификаторах
`CS_id` / `SD_id` / `code_1C`, что и его «сосед». Если не уверены, какой API использовать —
начните с хаба.

| Документ | Что это |
|----------|---------|
| **[SalesDoc API V2](README.md)** | *(этот документ)* Полная интеграция на чтение и запись (~80 методов GET/SET: справочники, заказы, склад, финансы, фото). |
| **[Country Sales API V2](https://github.com/Clever91/countrysale-api-docs/blob/main/ru/README.md)** | Узкий API только на чтение по страновой инсталляции: вход, филиалы, агенты, клиенты, визиты. |
