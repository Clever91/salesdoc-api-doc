...
| `filter.additionalStatus` | Массив ID дополнительных статусов; `0` — заказы без дополнительного статуса |
...
```
> Дата берётся из истории изменений заказа. Изменение дополнительного статуса заказа (в том числе его автоматическое снятие) также считается обновлением заказа и учитывается этим фильтром.
...
**17. Фильтр по дополнительным статусам (`filter.additionalStatus`):**
```json
{
    "method": "getOrder",
    "auth": { "userId": "d0_1", "token": "..." },
...
```
> Массив ID дополнительных статусов (справочник — [`getSubstatus`](06-get-extra.md#950-getsubstatus--справочник-дополнительных-статусов)). Значение `0` выбирает заказы **без** дополнительного статуса; значения можно комбинировать: `[0, 21]` — заказы без дополнительного статуса **или** с дополнительным статусом 21. Если фильтр не передан, выборка не ограничивается по дополнительному статусу (поведение полностью обратно совместимо).

**18. Комбинирование нескольких фильтров:**
```json
{
    "method": "getOrder",
    "auth": { "userId": "d0_1", "token": "..." },
...
| **additionalStatus** | object\|null | Дополнительный статус заказа; `null`, если дополнительный статус не установлен |
| `additionalStatus.id` | integer | Идентификатор дополнительного статуса (справочник — [`getSubstatus`](06-get-extra.md#950-getsubstatus--справочник-дополнительных-статусов)) |
| `additionalStatus.name` | string\|null | Название дополнительного статуса; `null`, если статус был удалён из справочника |
...
> **Примечание:** поле `additionalStatus` присутствует в каждом заказе: это объект `{id, name}`, если дополнительный статус установлен, или `null`, если у заказа нет дополнительного статуса. Если установленный статус был удалён из справочника, `id` возвращается, а `name` равно `null`.
...
[← Назад к оглавлению](README.md) | [Далее: Финансы →](05-get-finance.md)
